...
export async function unlockPDF(file: File, password: string): Promise<Blob> {
  ...
  try {
    ...
    if (subject && subject.includes('ENCRYPTED')) {
      // Extract stored password hash from metadata
    ...
    pdf.setSubject('');
    pdf.setKeywords([]);
    // Remove both emoji and non-emoji versions of protection prefix
  ...
  for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
    ...
    const imageBlob = await new Promise<Blob>((resolve) => {
      if (format === 'jpg') {
        ...
        // For PNG and TIFF (fallback to PNG)
...
// Duplicate function removed - using the one above at line 872
